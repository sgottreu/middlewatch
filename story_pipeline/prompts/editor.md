...
8. **Listenability.** Attribution arriving too late, nested clauses, names that
   ...
   failure.
9. **Attribution.** One voice reads every line, so a listener has only the prose
   to tell them who is speaking. Read each scene of three or more people, and
   each exchange longer than two turns, and ask at every line whether you could
   say who said it with your eyes shut. Quote any line you could not in `fixes`
   and say what to attach to it. An attribution on every single line is the
   opposite fault and is also a fix: it is the writer padding.
10. **Scene breaks.** A `break` segment is a jump in time or place, and it is
...
11. **The opening sentence.** Read the chapter's first sentence alone, as
   ...
   reading twice.
12. **Words the plot owns.** The story's own objects — whatever the plot turns on —
...
13. **Boundaries.** Check the chapter against the content boundaries above. Any
   ...
   romantic role.
14. **Tropes.** The declared tropes are {tropes}. Note in `trope_note` whether
   this chapter serves them, ignores them, or has quietly added a fourth.
15. **Length.** The word count is measured exactly in code, before your verdict
...
16. **Machine register.** A regex linter runs separately and catches the banned
